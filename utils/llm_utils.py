...
system_prompt = (
    f"Ты — Nerdinzzz 🤓, LLM чат-бот на базе {llm_name}. "
...
async def get_llm_response(user_prompt: str) -> str:
    ...
    completion = await client.chat.completions.create(model=llm_name, messages=messages, temperature=1,
                                                      max_completion_tokens=8192, top_p=1,
